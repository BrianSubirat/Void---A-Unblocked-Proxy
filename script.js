...
function closeSidePanel() {
  ...
  // Restore widgets and music player
  if (widgetsContainer) {
    widgetsContainer.style.display = 'block';
  }
  if (musicPlayer) {
    // Restore to previous visibility state
    const playerVisible = localStorage.getItem('playerVisible') !== 'false';
    musicPlayer.style.display = playerVisible ? 'flex' : 'none';
  }
  
  sidePanel.classList.remove('active');
  updateNotificationBtn.style.display = 'block';
}
...
function openCategory(category) {
  ...
  // Rest of the existing openCategory code remains the same...
